 ...
 */
function vonzot_add_menu_parent_class( $items ) {
...
add_filter( 'wp_nav_menu_objects', 'vonzot_add_menu_parent_class' );
 ...
 */
function vonzot_nav_menu_social_icons( $item_output, $item, $depth, $args ) {

	$social_icons = vonzot_social_links_icons();
 ...
 */
function vonzot_menu_item_markup( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'vonzot_menu_item_markup', 10, 4 );
 ...
 */
function vonzot_add_menu_item_link_attributes( $atts, $item, $args ) {
...
add_filter( 'nav_menu_link_attributes', 'vonzot_add_menu_item_link_attributes', 10, 3 );
 ...
 * @return array $classes
 */
function vonzot_add_menu_item_css_classes( $classes, $item, $args ) {
...
add_filter( 'nav_menu_css_class', 'vonzot_add_menu_item_css_classes', 10, 3 );

/**
 * Add a cart menu item in mobile menu
 *
 */
function vonzot_add_cart_menu_item_mobile( $items, $args ) {
	...
	if ( vonzot_display_cart_menu_item() ) {
		if ( $args->theme_location === 'primary' && preg_match( '/mobile/', $args->menu_class ) ) {
			...
			$items .= vonzot_cart_menu_item_mobile( false );
			$items .= '</li>';
		}
	}
...
add_filter( 'wp_nav_menu_items', 'vonzot_add_cart_menu_item_mobile', 10, 2 );
 ...
 * @param string $location
 */
function vonzot_get_menu_args( $location = 'primary', $type = 'desktop', $depth = 3 ) {
	...
	$args = array(
		...
		'depth'			=> $depth,
		'fallback_cb'		=> 'vonzot_menu_fallback',
 ...
 */
function vonzot_menu_fallback( $args ) {
	...
	$link = $link_before
	. '<a class="no-link-style" href="' . admin_url( 'nav-menus.php' ) . '">' . $before . esc_html__( 'Add Menu', 'vonzot' ) . $after . '</a>'
	. $link_after;
 ...
 */
function vonzot_social_links_icons() {
	...
	return apply_filters( 'vonzot_social_links_icons', $social_links_icons );
}