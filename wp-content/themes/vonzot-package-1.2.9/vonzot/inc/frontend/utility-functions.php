<?php
/**
 * Vonzot frontend utility functions
 *
 * @package WordPress
 * @subpackage Vonzot
 * @version 1.2.9
 ...
 */
function vonzot_get_current_url() {
	global $wp;
 ...
 */
function vonzot_get_last_post_id( $post_type = 'post' ) {
 ...
 */
function vonzot_content_width() {

	$content_width = 1140;

	$GLOBALS['content_width'] = apply_filters( 'vonzot_content_width', $content_width );
}
add_action( 'after_setup_theme', 'vonzot_content_width', 0 );

/**
 * WooCommerce AJAX search result
 */
function vonzot_ajax_search_query( $typed = null ) {
...
if ( ! function_exists( 'vonzot_get_image_dominant_color' ) ) {
	/**
	 * Get dominant color image
	 *
	 * @param int $attachment_id
	 */
	function vonzot_get_image_dominant_color( $attachment_id ) {
 ...
 * @return string
 */
function vonzot_sanitize_style_attr( $style ) {
	return esc_attr( trim( vonzot_compact_css( $style ) ) );
}
 ...
 * @return string
 */
function vonzot_sample( $text, $num_words = 55, $more = '...' ) {
	$text = wp_strip_all_tags( wp_trim_words( vonzot_clean_post_content( $text ), $num_words, $more ) );
 ...
 */
function vonzot_get_video_url_type( $url ) {
 ...
 * @return string $color
 */
function vonzot_sanitize_color( $color ) {
 ...
 */
function vonzot_sanitize_html_classes( $class, $fallback = null ) {
	if ( is_string( $class ) ) {
		$class = explode( ' ', $class);
	}
 ...
 * @return string
 */
function vonzot_esc_style_attr( $style ) {

	return esc_attr( trim( vonzot_clean_spaces( $style ) ) );
}
 ...
 * @return $string
 */
function vonzot_clean_post_content( $string ) {
 ...
 */
function vonzot_shortcode_preg_match( $shortcode, $content = '' ) {
 ...
 * @return string $html
 */
function vonzot_filter_audio_shortcode_output( $html ) {

	return '<div class="audio-shortcode-container">' . $html . '</div>';
}
add_filter( 'wp_audio_shortcode', 'vonzot_filter_audio_shortcode_output' );
 ...
 * @return string $html
 */
function vonzot_filter_oembed_output( $html, $url, $attr, $post_id ) {
...
add_filter( 'embed_oembed_html', 'vonzot_filter_oembed_output', 10, 4 );
 ...
 * @return string $html
 */
function vonzot_filter_video_shortcode_output( $html ) {

	return '<div class="video-shortcode-container">' . $html . '</div>';
}
add_filter( 'wp_video_shortcode', 'vonzot_filter_video_shortcode_output' );

if ( ! function_exists( 'vonzot_format_number' ) ) {
	/**
	 ...
	 * @return string
	 */
	function vonzot_format_number( $n ) {
 ...
 */
function vonzot_get_color_brightness( $hex ) {
 ...
 * @return string light|dark
 */
function vonzot_get_color_tone( $hex, $index = 210 ) {
 ...
 * @return bool
 */
function vonzot_color_is_black( $hex ) {
 ...
 * @return bool
 */
function vonzot_color_is_white( $hex ) {
 ...
 * @return string
 */
function vonzot_color_brightness( $hex, $percent ) {

	$steps = ( ceil( ( $percent * 200 ) / 100 ) ) * 2;
	$steps = max( -255, min( 255, $steps ) );
	$hex = str_replace( '#', '', vonzot_sanitize_color( $hex ) );
	if ( strlen( $hex ) === 3 ) {
 ...
 */
function vonzot_adjust_brightness( $hexCode, $adjustPercent ) {
 ...
 * @return string $value
 */
function vonzot_sanitize_css_value( $value, $default_unit = 'px', $default_value = '1' ) {
