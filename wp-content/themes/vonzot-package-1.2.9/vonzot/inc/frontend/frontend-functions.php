 ...
 */
function vonzot_get_hero_background() {

	$hero_bg_type = vonzot_get_inherit_mod( 'hero_background_type', apply_filters( 'vonzot_default_hero_background_type', 'featured-image' ) );

	if ( is_404() || 'none' === vonzot_get_inherit_mod( 'hero_layout' ) || 'none' === $hero_bg_type ) {
		return;
	}

	$post_id = vonzot_get_inherit_post_id();
	$overlay = vonzot_get_inherit_mod( 'hero_overlay' );
	$output = '';

	if ( 'image' === $hero_bg_type ) {

		$background_img = get_post_meta( $post_id, '_post_hero_background_img', true );
		$background_img_url = vonzot_get_url_from_attachment_id( $background_img, 'vonzot-XL' );
		...
		$background_effect = ( get_post_meta( $post_id, '_post_hero_background_effect', true ) ) ? get_post_meta( $post_id, '_post_hero_background_effect', true ) : vonzot_get_theme_mod( 'post_header_effect' );
		...
		$output .= vonzot_background_img( $img_bg_args );
		...
		$output .= vonzot_background_video( $video_bg_args, true );
		...
		$output .= vonzot_background_slideshow( $slideshow_args );
		...
		if ( vonzot_is_home_as_blog() || 'featured-image' !== $hero_bg_type ) {

			$bg_img_id = vonzot_get_hero_image_id();

		} else {
			$bg_img_id = vonzot_get_featured_image_id( vonzot_get_inherit_post_id(), true );
		}
		...
		$img_bg_args = array(
			...
			'background_effect' => vonzot_get_inherit_mod( 'hero_background_effect' ),
		);
	...
	if ( 'none' !== $overlay && $output ) {
		...
		if ( 'custom' === $overlay ) {
			$overlay_color = vonzot_get_inherit_mod( 'hero_overlay_color' );
			$overlay_opacity = vonzot_get_inherit_mod( 'hero_overlay_opacity' );
 ...
 * @return string
 */
function vonzot_get_header_font_tone() {

	$default_font_tone = ( preg_match( '/dark/', vonzot_get_theme_mod( 'color_scheme' ) ) ) ? 'light' : 'dark';
	$header_font_tone = ( vonzot_has_hero() ) ? 'light' : $default_font_tone;
	$header_font_tone = apply_filters( 'vonzot_default_hero_font_tone', $header_font_tone );

	if ( 'none' === vonzot_get_inherit_mod( 'hero_layout' ) && 'solid' !== vonzot_get_inherit_mod( 'menu_style' ) ) {
		$header_font_tone = 'light';
		$header_font_tone = apply_filters( 'vonzot_default_no_header_hero_font_tone', $header_font_tone );
	}
	...
	if ( get_post_meta( vonzot_get_inherit_post_id(), '_post_hero_font_tone', true ) ) {
		$header_font_tone = get_post_meta( vonzot_get_inherit_post_id(), '_post_hero_font_tone', true );
	}

	return apply_filters( 'vonzot_hero_font_tone', $header_font_tone );
}
 ...
 * @return string
 */
function vonzot_get_menu_layout() {

	$menu_layout = vonzot_get_inherit_mod( 'menu_layout', 'top-right' );

	$post_types = apply_filters( 'vonzot_no_header_post_types', array( 'product', 'release', 'event', 'proof_gallery', 'attachment' ) );
	...
	return apply_filters( 'vonzot_menu_layout', $menu_layout );
}
 ...
 * @return string
 */
function vonzot_get_menu_style() {

	$menu_style = vonzot_get_inherit_mod( 'menu_style' );
	...
	if ( is_single() ) {
		...
		if ( ! $post_menu_style_meta && 'none' === vonzot_get_inherit_mod( $post_type . '_hero_layout' ) ) {
			$menu_style = 'solid';
		}
	}

	return apply_filters( 'vonzot_menu_style', $menu_style );
}

/**
 * Get type of content in the menu call to action area (shop icons, social icons etc...)
 */
function vonzot_get_menu_cta_content_type() {

	$content_type = vonzot_get_inherit_mod( 'menu_cta_content_type', 'none' );

	return apply_filters( 'vonzot_menu_cta_content_type', $content_type );
}
 ...
 */
function vonzot_get_single_post_layout( $post_id = null, $default = 'default' ) {

	$post_id = ( $post_id ) ? $post_id : get_the_ID();

	$single_post_layout = vonzot_get_theme_mod( 'post_single_layout' );
	$single_post_layout_meta = get_post_meta( $post_id, '_post_layout', true );

	if ( is_singular( 'artist' ) ) {
		$single_post_layout = vonzot_get_theme_mod( 'artist_single_layout' );
	}
	...
	if ( is_singular( 'post' ) && vonzot_is_vc() && $single_post_layout_meta !== 'sidebar-left' && $single_post_layout_meta !== 'sidebar-right' ) {
		$single_post_layout = 'default';
	}

	return apply_filters( 'vonzot_single_post_layout', $single_post_layout );
}

/**
 * Single post WVC layout
 */
function vonzot_get_single_post_wvc_layout( $post_id = null ) {
	...
	if ( is_singular( 'post' ) ) {
		...
		if ( vonzot_is_vc() ) {
			...
			return apply_filters( 'vonzot_single_post_wvc_layout', $single_post_wvc_layout );
		}
	}
}
 ...
 * @return string $layout
 */
function vonzot_get_single_video_layout( $post_id = null, $default = 'default' ) {

	$post_id = ( $post_id ) ? $post_id : get_the_ID();

	$single_post_layout = vonzot_get_theme_mod( 'video_single_layout' );
	...
	/* Force full width layout on posts that use page builder */
	if ( is_singular( 'video' ) && vonzot_is_vc() ) {
		$single_post_layout = 'default';
	}

	return apply_filters( 'vonzot_single_video_layout', $single_post_layout );
}
 ...
 * @return string $layout
 */
function vonzot_get_single_mp_event_layout( $post_id = null, $default = 'default' ) {

	$post_id = ( $post_id ) ? $post_id : get_the_ID();

	$single_post_layout = vonzot_get_theme_mod( 'mp_event_single_layout' );
	...
	/* Force full width layout on posts that use page builder */
	if ( is_singular( 'mp_event' ) && vonzot_is_vc() ) {
		$single_post_layout = 'default';
	}

	return apply_filters( 'vonzot_single_mp_event_layout', $single_post_layout );
}
 ...
 * @return string $layout
 */
function vonzot_get_single_mp_column_layout( $post_id = null, $default = 'default' ) {

	$post_id = ( $post_id ) ? $post_id : get_the_ID();

	$single_post_layout = vonzot_get_theme_mod( 'mp_column_single_layout' );
	...
	/* Force full width layout on posts that use page builder */
	if ( is_singular( 'mp_column' ) && vonzot_is_vc() ) {
		$single_post_layout = 'default';
	}

	return apply_filters( 'vonzot_single_mp_column_layout', $single_post_layout );
}
 ...
 * @return string $layout
 */
function vonzot_get_single_artist_layout( $post_id = null, $default = 'default' ) {
	...
	/* Force full width layout on posts that use page builder */
	if ( is_singular( 'artist' ) && vonzot_is_vc() ) {
		$single_post_layout = 'default';
	}

	return apply_filters( 'vonzot_single_artist_layout', $single_post_layout );
}
 ...
 * @return string URL
 */
function vonzot_get_hero_image_id() {
 ...
 * @return int
 */
function vonzot_get_featured_image_id( $post_id = null, $fallback = false ) {
	...
	if ( vonzot_get_hero_image_id() && $fallback ) {
		return vonzot_get_hero_image_id();
	}
}
 ...
 */
function vonzot_get_gallery_params() {

	$array = array();

	if ( vonzot_get_post_gallery_ids() ) {

		$vonzot_get_post_gallery_ids = vonzot_list_to_array( vonzot_get_post_gallery_ids() );

		foreach ( $vonzot_get_post_gallery_ids as $attachment_id ) {

			$attachment = get_post( $attachment_id );

			if ( $attachment ) {
				$src = esc_url( vonzot_get_url_from_attachment_id( $attachment_id, 'vonzot-XL' ) );
 ...
 * @param int $post_id
 */
function vonzot_get_first_category( $post_id = null ) {
 ...
 * @param int $post_id
 */
function vonzot_get_first_category_url( $post_id = null ) {
 ...
 * @return string URL
 */
function vonzot_get_hero_image( $args ) {
	...
	$post_id = ( $in_loop ) ?  get_the_ID() : vonzot_get_inherit_post_id();
 ...
 * @return string URL
 */
function vonzot_get_hero_image_src( $in_loop = true ) {
 ...
 * @return string
 */
function vonzot_get_first_url( $post_id = null ) {

	if ( is_search() || vonzot_is_woocommerce_page() ) {
		return;
	}
 ...
 * @return string
 */
function vonzot_get_first_video_url( $post_id = null ) {
 ...
 * @return string
 */
function vonzot_get_first_mp3_url( $post_id = null ) {
 ...
 */
function vonzot_get_first_quote( $post_id = null ) {
 ...
 * @return string $link
 */
function vonzot_remove_more_jump_link( $link )  {
	$offset = strpos( $link, '#more-' );
	if ( $offset ) { $end = strpos( $link, '"',$offset ); }
	if ( $end ) { $link = substr_replace( $link, '', $offset, $end-$offset ); }
	return $link;
}
add_filter( 'the_content_more_link', 'vonzot_remove_more_jump_link' );

/**
 * Get blog index ID
 */
function vonzot_get_blog_index_id() {
 ...
 * Get events page_id
 */
function vonzot_get_portfolio_page_id() {
 ...
 * Get portfolio URL
 */
function vonzot_get_portfolio_url() {
	if ( function_exists( 'wolf_portfolio_get_page_id' ) ) {
 ...
 * Get events page_id
 */
function vonzot_get_events_page_id() {
 ...
 * Get events URL
 */
function vonzot_get_events_url() {
	if ( function_exists( 'wolf_events_get_page_id' ) ) {
 ...
 * Get albums URL
 */
function vonzot_get_albums_url() {
 ...
 */
function vonzot_get_discography_page_id() {
 ...
 * Get discography URL
 */
function vonzot_get_discography_url() {
	if ( function_exists( 'wolf_discography_get_page_id' ) ) {
 ...
 * Get videos page_id
 */
function vonzot_get_videos_page_id() {
 ...
 * Get videos URL
 */
function vonzot_get_videos_url() {
	if ( function_exists( 'wolf_videos_get_page_id' ) ) {
 ...
 * Get artists URL
 */
function vonzot_get_artists_url() {
 ...
 * Get artists page_id
 */
function vonzot_get_artists_page_id() {
 ...
 * Get post type name
 */
function vonzot_get_post_type_name() {
 ...
 * Get placeholder URL
 */
function vonzot_get_placeholder_url() {
	$rand = rand( 1, 1084 );
 ...
 */
function vonzot_get_instagram_image_url( $post_id = null ) {
	
	if ( ! $post_id ) {
		$post_id = get_the_ID();
	}

	$remote_url = 'https://api.instagram.com/oembed/?url=' . vonzot_get_first_url( $post_id );
 ...
 */
function vonzot_get_instagram_image( $post_id = null, $link = true ) {
	...
	if ( ! is_wp_error( $response ) && is_array( $response ) ) {
		...
		if ( $thumbnail_url ) {
			return '<a class="instagram-image" href="' . esc_url( vonzot_get_first_url( $post_id ) ) . '" title="' . esc_attr( get_the_title( $post_id ) ) . '" target="_blank">
 ...
 */
function vonzot_get_post_gallery_ids() {
	...
	if ( preg_match( '/images="[0-9 ,]+"/', $content, $match ) ) {
		...
		if ( $gallery && isset( $gallery['ids'] ) ) {
			...
			$ids = vonzot_array_to_list( $ids_array );
		}
	}
 ...
 * @return int
 */
function vonzot_get_first_gallery_image_count() {

	if ( vonzot_get_post_gallery_ids() ) {
		return absint( count( vonzot_list_to_array( vonzot_get_post_gallery_ids() ) ) );
	}
}
 ...
 */
function vonzot_oembed_add_args( $provider, $url, $args ) {
	...
	if ( strpos( $provider, 'vimeo.com' ) ) {
		$provider = add_query_arg( array(
			...
			'badge' => '0',
			'byline' => '0',
			'color' => str_replace( '#', '', apply_filters( 'vonzot_vimeo_accent_color', vonzot_get_inherit_mod( 'accent_color' ) ) ),
		), $provider );
	}
...
add_filter( 'oembed_fetch_url', 'vonzot_oembed_add_args', 10, 3 );
 ...
 * @return bool $bool
 */
function vonzot_parallax_mobile( $bool ) {

	if ( vonzot_get_theme_mod( 'enable_mobile_parallax' ) ) {
		$bool = false;
	}

	return $bool;
}
add_filter( 'vonzot_parallax_no_ios', 'vonzot_parallax_mobile', 40 );
add_filter( 'vonzot_parallax_no_android', 'vonzot_parallax_mobile', 40 );
add_filter( 'vonzot_parallax_no_small_screen', 'vonzot_parallax_mobile', 40 );
add_filter( 'wvc_parallax_no_ios', 'vonzot_parallax_mobile', 40 );
add_filter( 'wvc_parallax_no_android', 'vonzot_parallax_mobile', 40 );
add_filter( 'wvc_parallax_no_small_screen', 'vonzot_parallax_mobile', 40 );
 ...
 * @return bool $bool
 */
function vonzot_animation_mobile( $bool ) {

	if ( vonzot_get_theme_mod( 'enable_mobile_animations' ) ) {
		$bool = true;
	}

	return $bool;
}
add_filter( 'vonzot_force_animation_mobile', 'vonzot_animation_mobile', 40 );
add_filter( 'wvc_force_animation_mobile', 'vonzot_animation_mobile', 40 );
 ...
 */
function vonzot_overwrite_lightbox( $lightbox ) {
	return 'fancybox';
}
add_filter( 'wvc_lightbox', 'vonzot_overwrite_lightbox', 40 );

/**
 * Site footer class
 */
function vonzot_set_site_footer_tone_class( $class ) {

	$body_bg_color = vonzot_get_theme_mod( 'body_background_color' );
...
add_filter( 'vonzot_site_footer_class', 'vonzot_set_site_footer_tone_class' );