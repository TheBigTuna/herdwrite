 ...
 *
 * A function that returns any loop of any post type and push it to the vonzot_posts hook
 * Cool right?
 *
 * @package WordPress
 * @subpackage Vonzot
 ...
 * Output posts
 */
function vonzot_output_posts( $atts ) {
	...
	$atts = wp_parse_args( $atts, array(
		...
		'posts_per_page' => -1,
		'paged' => null,
		'grid_padding' => vonzot_get_theme_mod( 'post_grid_padding', 'yes' ),
		'item_animation' => vonzot_get_theme_mod( 'post_item_animation' ),
		'columns' => 'default',
		'include_ids' => '',
		'exclude_ids' => '',
		...
		'post_index' => false,
		'post_display' => vonzot_get_theme_mod( 'post_display', 'standard' ),
		'post_metro_pattern' => 'auto',
		'post_module' => 'grid',
		'post_thumbnail_size' => 'standard',
		'post_layout' => 'standard',
		'post_category_filter' => vonzot_get_theme_mod( 'post_category_filter', false ),
		'post_excerpt_type' => 'auto', // for standard post type
		'post_excerpt_length' => 'shorten',
		'post_display_elements' => vonzot_get_theme_mod( 'post_display_elements', 'show_thumbnail,show_date,show_text,show_author,show_category' ),
		...
		'page_by_parent' => '',
		'work_index' => false,
		'work_display' => vonzot_get_theme_mod( 'work_display', 'grid' ),
		'work_module' => 'grid',
		'work_thumbnail_size' => 'standard',
		'work_layout' => 'overlay',
		'work_is_gallery' => '',
		'work_category_filter' => vonzot_get_theme_mod( 'work_category_filter', true ),
		'work_category_filter_text_alignment' => 'center',
		'work_hover_effect' =>  apply_filters( 'work_default_hover_effect', 'default' ),
		'work_type_include' => '',
		'work_type_exclude' => '',
		'product_index' => false,
		'product_display' => vonzot_get_theme_mod( 'product_display', 'grid_classic' ),
		...
		'order' => '',
		'release_index' => false,
		'release_display' => vonzot_get_theme_mod( 'release_display', 'grid' ),
		'release_metro_pattern' => 'pattern-1',
		'release_hover_effect' =>  apply_filters( 'release_default_hover_effect', 'default' ),
		'release_category_filter' => vonzot_get_theme_mod( 'release_category_filter', false ),
		...
		'event_index' => false,
		'event_display' => vonzot_get_theme_mod( 'event_display', 'list' ),
		'event_module' => 'grid',
		...
		'mp_event_display' => vonzot_get_theme_mod( 'mp_event_display', 'grid' ),
		'mp_event_module' => 'grid',
		'mp_event_category_include' => '',
		'mp_event_category_exclude' => '',
		'video_index' => false,
		'video_display' => vonzot_get_theme_mod( 'video_display', 'grid' ),
		'video_layout' => 'overlay',
		'video_module' => 'grid',
		'video_preview' => false,
		'video_onclick' => vonzot_get_theme_mod( 'video_onclick', 'lightbox' ),
		'video_category_filter' => vonzot_get_theme_mod( 'video_category_filter', false ),
		...
		'attachment_display' => vonzot_get_theme_mod( 'attachment_display', 'masonry_horizontal' ),
	...
	$atts = apply_filters( 'vonzot_post_module_atts', $atts );
	...
	$layout = apply_filters( 'vonzot_post_module_layout', $layout, $atts );
	...
	$display = apply_filters( 'vonzot_post_module_display', $display, $atts );
	$metro_pattern = ( isset( ${$post_type . '_metro_pattern'} ) ) ? ${$post_type . '_metro_pattern'} : 'auto';
	$metro_pattern = apply_filters( 'vonzot_post_module_metro_pattern', $metro_pattern, $atts );
	$module = ( isset( ${$post_type . '_module'} ) ) ? ${$post_type . '_module'} : 'grid';
	$module = apply_filters( 'vonzot_post_module_module', $module, $atts );
	$category_filter = ( isset( ${$post_type . '_category_filter'} ) ) ? vonzot_attr_bool( ${$post_type . '_category_filter'} ) : false;
	
	$thumbnail_size = ( isset( ${$post_type . '_thumbnail_size'} ) ) ? ${$post_type . '_thumbnail_size'} : 'standard';
	
	$is_index = ( isset( ${$post_type . '_index'} ) ) ? vonzot_attr_bool( ${$post_type . '_index'} ) : false;
	...
	$args['post__in'] = array();
	if ( $include_ids ) {
		$args['post__in'] = vonzot_list_to_array( $include_ids );
	}
	$exclude_ids_array = array();
	$exclude_ids_array[] = vonzot_get_the_ID(); // exclude current post, obviously or the internet will explode
	...
	if ( $exclude_ids ) {
		$exclude_ids_array = array_merge( $exclude_ids_array, vonzot_list_to_array( $exclude_ids ) );
	}
	...
	if ( 'post' === $post_type ) {
		...
		if ( $category  ) {
			$args['category_name'] = vonzot_clean_list( $category );
		}

		if ( $category_exclude ) {
			$args['category__not_in'] = array( vonzot_clean_list( $category_exclude ) );
		}
		if ( $tag ) {
			$args['tag'] = vonzot_clean_list( $tag );
		}

		if ( $tag_exclude ) {
			$args['tag__not_in'] = vonzot_clean_list( $tag_exclude );
		}
	...
	if ( 'work' === $post_type ) {
		if ( $work_type_include ) {
			$args['work_type'] = vonzot_clean_list( $work_type_include );
		}
		if ( $work_type_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'work_type',
					'terms' => array( vonzot_clean_list( $work_type_exclude ) ),
					'field' => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	if ( 'video' === $post_type ) {
		...
		if ( $video_type_include ) {
			$args['video_type'] = vonzot_clean_list( $video_type_include );
		}
		if ( $video_type_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'video_type',
					'terms' => array( vonzot_clean_list( $video_type_exclude ) ),
					'field' => 'slug',
					'operator' => 'NOT IN',
				),
			);
		...
		if ( $video_tag_include ) {
			$args['video_tag'] = vonzot_clean_list( $video_tag_include );
		}
		if ( $video_tag_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'video_tag',
					'terms' => array( vonzot_clean_list( $video_tag_exclude ) ),
					'field' => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	if ( 'mp-event' === $post_type ) {
		if ( $mp_event_category_include ) {
			$args['mp_event_category_include'] = vonzot_clean_list( $mp_event_category_include );
		}
		if ( $mp_event_category_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'mp-event_category',
					'terms' => array( vonzot_clean_list( $mp_event_category_exclude ) ),
					'field' => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	if ( 'gallery' === $post_type ) {
		if ( $gallery_type_include ) {
			$args['gallery_type'] = vonzot_clean_list( $gallery_type_include );
		}
		if ( $gallery_type_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'gallery_type',
					'terms' => array( vonzot_clean_list( $gallery_type_exclude ) ),
					'field' => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	if ( 'release' === $post_type ) {
		if ( $band_include ) {
			$args['band'] = vonzot_clean_list( $band_include );
		}
		if ( $band_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'band',
					'terms' => array( vonzot_clean_list( $band_exclude ) ),
					'field' => 'slug',
					'operator' => 'NOT IN',
				),
			);
		...
		if ( $label_include ) {
			$args['label'] = vonzot_clean_list( $label_include );
		}
		if ( $label_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'label',
					'terms' => array( vonzot_clean_list( $label_exclude ) ),
					'field' => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	if ( 'artist' === $post_type ) {
		if ( $artist_genre_include ) {
			$args['artist_genre'] = vonzot_clean_list( $artist_genre_include );
		}
		if ( $artist_genre_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'artist_genre',
					'terms' => array( vonzot_clean_list( $artist_genre_exclude ) ),
					'field' => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	if ( 'event' === $post_type ) {
		...
		if ( $artist_include ) {
			$args['we_artist'] = vonzot_clean_list( $artist_include );
		}
		if ( $artist_exclude ) {
			$args['tax_query'] = array(
				array(
					'taxonomy' => 'we_artist',
					'terms' => array( vonzot_clean_list( $artist_exclude ) ),
					'field' => 'slug',
					'operator' => 'NOT IN',
				),
			);
	...
	if ( 'product' === $post_type ) {
		...
		if ( $product_cat ) {
			$args['product_cat'] = vonzot_clean_list( $product_cat );
		}
	...
	if ( 'attachment' === $post_type ) {
		...
		if ( isset( $_GET['s'] ) ) {
			...
			$tax_query = new WP_Query( apply_filters( 'vonzot_post_module_query_args', $args ) );
	...
	if ( $is_index ) { // is index page
		...
		add_filter( 'found_posts', 'vonzot_offset_pagination_fix', 10, 2 );
		...
		remove_filter( 'found_posts', 'vonzot_offset_pagination_fix', 10, 2 );
	}
	if ( $query->have_posts() ) {
		...
		if ( $category_filter ) {
			...
			set_query_var( 'filter_args', array(
				...
				/* Post */
				'category' => vonzot_clean_list( $category ),

				/* Work */
				'work_type_include' => vonzot_clean_list( $work_type_include ),
				'work_type_exclude' => vonzot_clean_list( $work_type_exclude ),
				'work_category_filter_text_alignment' => $work_category_filter_text_alignment,

				/* Albums */
				'gallery_type_include' => vonzot_clean_list( $gallery_type_include ),
				'gallery_type_exclude' => vonzot_clean_list( $gallery_type_exclude ),
				'gallery_category_filter_text_alignment' => $gallery_category_filter_text_alignment,

				/* Video */
				'video_type_include' => vonzot_clean_list( $video_type_include ),
				'video_type_exclude' => vonzot_clean_list( $video_type_exclude ),
				'video_category_filter_text_alignment' => $video_category_filter_text_alignment,

				/* Artist */
				'artist_genre_include' => vonzot_clean_list( $artist_genre_include ),
				'artist_genre_exclude' => vonzot_clean_list( $artist_genre_exclude ),
				'artist_category_filter_text_alignment' => $artist_category_filter_text_alignment,

				/* Release */
				'band_include' => vonzot_clean_list( $band_include ),
				'band_exclude' => vonzot_clean_list( $band_exclude ),
		...
		$tag = ( $is_list ) ? 'ul' : 'div';
		echo '<' . $tag . ' id="' . esc_attr( $id ) . '" data-post-type="' . esc_attr( $post_type ) . '" data-params="' . esc_js( $json_params ) . '" class="' . vonzot_sanitize_html_classes( $class ) . '"';
		
		if ( vonzot_esc_style_attr( $inline_style ) ) {
			echo ' style="' . vonzot_esc_style_attr( $inline_style ) . '"';
		}
		 ...
		 */
		add_filter( 'vonzot_post_display', function( $default ) use ( $display ) { return $display; } );
		add_filter( 'vonzot_post_module_layout', function( $default ) use ( $layout ) { return $layout; } );
		add_filter( 'vonzot_post_item_animation', function( $default ) use ( $item_animation ) { return vonzot_animation_fallback( $item_animation ); } );
		add_filter( 'vonzot_post_columns', function( $default ) use ( $columns ) { return $columns; } );
		add_filter( 'vonzot_post_force_loop_class', function() { return true; } );
		...
		echo '</' . $tag . '><!--.items-->';
		remove_all_filters( 'vonzot_post_display' );
		remove_all_filters( 'vonzot_post_item_animation' );
		remove_all_filters( 'vonzot_post_columns' );
		remove_all_filters( 'vonzot_post_module_layout' );
		if ( 'none' !== $pagination ) {
			...
			do_action( 'vonzot_pagination', $query, $pagination_args  );
		}
		...
		if ( 'event' === $post_type ) {
			...
			if ( 'past' === $timeline ) {
				...
				apply_filters( 'vonzot_no_past_event_text', esc_html__( 'No past event.', 'vonzot' ) );

			} else {
				if ( function_exists( 'we_get_option' ) ) {
					echo we_get_option( 'no_event_text', esc_html__( 'No upcoming shows scheduled.', 'vonzot' ) );
				}
			}

		} elseif ( 'release' === $post_type ) {

			echo apply_filters( 'vonzot_no_release_text', esc_html__( 'No release yet.', 'vonzot' ) );

			if ( is_user_logged_in() ) {
				echo '&nbsp;';
				echo apply_filters( 'vonzot_no_release_user_text', esc_html__( 'Is there a featured image set for each of your release?', 'vonzot' ) );
			}

		} elseif ( 'video' === $post_type ) {

			echo apply_filters( 'vonzot_no_video_text', esc_html__( 'No video yet.', 'vonzot' ) );

			if ( is_user_logged_in() ) {
				echo '&nbsp;';
				echo apply_filters( 'vonzot_no_video_user_text', esc_html__( 'Is there a featured image set for each of your video?', 'vonzot' ) );
			}

		} elseif ( 'work' === $post_type ) {

			echo apply_filters( 'vonzot_no_work_text', esc_html__( 'No work yet.', 'vonzot' ) );
...
add_action( 'vonzot_posts', 'vonzot_output_posts' );