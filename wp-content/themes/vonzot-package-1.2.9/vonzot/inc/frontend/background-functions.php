 ...
 */
function vonzot_background_overlay( $args ) {
	...
	if ( ( 'custom' === $overlay_color || 'auto' === $overlay_color ) && $overlay_custom_color ) {
		...
		$overlay_style .= 'background-color:' . vonzot_sanitize_color( $overlay_custom_color ) . ';';
	...
	return '<div style="' . vonzot_esc_style_attr( $overlay_style ) . '" class="' . vonzot_sanitize_html_classes( $class ) . '"></div><!--.wvc-bg-overlay-->';
}
 ...
 * @param array $args
 */
function vonzot_background_img( $args = array() ) {
	...
	$do_object_fit = ( wp_attachment_is_image( $background_img ) && 'no-repeat' === $background_repeat && 'default' !== $background_size && 'parallax' !== $background_effect && ! vonzot_is_edge() && ! wp_is_mobile() );
	...
	if ( $do_object_fit ) {
		...
		$src = vonzot_get_url_from_attachment_id( $background_img, $background_img_size );
		...
		$img_dominant_color = vonzot_get_image_dominant_color( $background_img );
		...
		if ( $background_img_lazyload ) {
			$cover_class .= ' lazy-hidden lazyload-bg ';
		}
		...
		if ( $img_dominant_color ) {
			$img_dominant_color = vonzot_sanitize_color( $img_dominant_color );
			$container_style .= "background-color:$img_dominant_color;";
		}

		$output .= '<div class="' . vonzot_sanitize_html_classes( $container_class ) . '" style="' . vonzot_esc_style_attr( $container_style ) . '">';
		...
		$output .= '<img
			...
			style="' . vonzot_esc_style_attr( $cover_style ) . '"
		...
		$output .= ' class="' . vonzot_sanitize_html_classes( $cover_class ) . '"
		...
		if ( 'parallax' === $background_effect && wp_attachment_is_image( $background_img ) ) {
			...
			$background_color = vonzot_get_image_dominant_color( $background_img );
		...
		if ( 'parallax' !== $background_effect ) {
			...
			if ( wp_attachment_is_image( $background_img ) ) {
				$background_img_url = vonzot_get_url_from_attachment_id( $background_img, $background_img_size );
				$style .= 'background-image:url(' . esc_url( $background_img_url ) . ');';
			}
		}

		$output .= '<div ' . $attrs . ' class="' . vonzot_sanitize_html_classes( $container_class ) . '" style="' . esc_attr( $style ) . '"></div>';

	} elseif ( vonzot_is_instagram_post() ) {

		$container_class = 'img-bg';

		$instagram_url = vonzot_get_instagram_image_url();
		$output .= '<div class="' . vonzot_sanitize_html_classes( $container_class ) . '" style="background-size:cover;background-image:url(' . esc_url( $instagram_url ) . ');"></div>';
		...
		$bg_fallback_url = vonzot_get_placeholder_url();

		$output .= '<div class="' . vonzot_sanitize_html_classes( $container_class ) . '" style="background-size:cover;background-image:url(' . esc_url( $bg_fallback_url ) . ');"></div>';
	}
 ...
 * @return string $output
 */
function vonzot_background_slideshow( $args = array() ) {

	extract( wp_parse_args( $args, array(
		'slideshow_image_size' => 'vonzot-XL',
		'slideshow_img_ids' => '',
	...
	$do_object_fit = ( ! vonzot_is_edge() && ! wp_is_mobile() );
	...
	if ( array() != $slideshow_img_ids && is_array( $slideshow_img_ids ) ) {
		...
		foreach ( $slideshow_img_ids as $image_id ) {
			...
			$src  = esc_url( vonzot_get_url_from_attachment_id( $image_id, $slideshow_image_size ) );
 ...
 */
function vonzot_background_video( $args = array(), $vimeo = true ) {

	$args = wp_parse_args( $args, array(
		'video_bg_url' => vonzot_get_first_video_url(),
	...
	if ( 'selfhosted' === vonzot_get_video_url_type( $args['video_bg_url'] ) ) {

		$output .= vonzot_video_bg( $args );

	} elseif ( 'youtube' === vonzot_get_video_url_type( $args['video_bg_url'] ) ) {

		$output .= vonzot_youtube_video_bg( $args );

	} elseif ( 'vimeo' === vonzot_get_video_url_type( $args['video_bg_url'] ) && $vimeo ) {

		$output .= vonzot_vimeo_video_bg( $args );
	}
 ...
 * @return string $output
 */
function vonzot_youtube_video_bg( $args ) {
	...
	wp_enqueue_script( 'vonzot-youtube-video-background' );
 ...
 * @return string $output
 */
function vonzot_vimeo_video_bg( $args ) {
	...
	wp_enqueue_script( 'vonzot-vimeo' );
