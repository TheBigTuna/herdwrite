<?php
/**
 * Vonzot featured media function
 *
 * @package WordPress
 * @subpackage Vonzot
 * @version 1.2.9
 ...
 */
function vonzot_featured_media( $post_id = null ) {
	$output = '';
	...
	if ( $has_post_thumbnail ) {
		...
		$default_thumbnail .= '<div class="entry-thumbnail">';
			$default_thumbnail .= '<a href="' . esc_url( $permalink ) . '" class="entry-link" title="' . esc_attr( sprintf( __( 'Permalink to %s', 'vonzot' ), the_title_attribute( 'echo=0' ) ) ) . '">';
	...
	if ( 'standard' === $post_format || 'chat' === $post_format ) {
		...
		if ( vonzot_is_instagram_post( $post_id ) ) {

			/**
			 * Embed Instagram
			 */
			$output .= vonzot_get_instagram_image( $post_id );
			 ...
			 * Featured image
			 */
			$large_img_src = get_the_post_thumbnail_url( '', 'vonzot-XL' );
			$output .= '<div class="entry-thumbnail">';
			$output .= '<a href="' . esc_url( $large_img_src ) . '" data-fancybox="' . esc_attr( get_the_title( $post_id ) ) . '" class="lightbox" title="' .  the_title_attribute( array( 'echo' => false, 'post' => $post_id ) ) . '">';
		...
		if ( vonzot_entry_slider() ) {

			$output .= vonzot_entry_slider();
		...
		if ( ! vonzot_is_playlist_audio_player( $post_id ) ) {
			$output .= $default_thumbnail;
		}

		/* Highlight the audio file. */
		if ( vonzot_get_audio_embed_content( $post_id ) ) {
			$output .= vonzot_get_audio_embed_content( $post_id );
		}

	} elseif ( 'quote' === $post_format ) {

		if ( vonzot_get_first_quote( $post_id ) ) {
			$output .= '<div class="entry-thumbnail">';
				$output .= '<a href="' . esc_url( $permalink ) . '" class="entry-link-mask" title="' . esc_attr( sprintf( __( 'Permalink to %s', 'vonzot' ), the_title_attribute( 'echo=0' ) ) ) . '"></a>';
				$output .= vonzot_background_img();

				$output .= '<div class="entry-summary-overlay">';
					$output .= '<blockquote class="entry-featured-quote">';
						$output .= vonzot_get_first_quote( $post_id );
		...
		if ( vonzot_get_first_url( $post_id ) ) {
			$output .= '<div class="entry-thumbnail">';
				$output .= '<a href="' . esc_url( vonzot_get_first_url( $post_id ) ) . '" target="_blank" class="entry-link-mask"></a>';
				$output .= vonzot_background_img();
				$output .= '<div class="entry-summary-overlay">';
			...
			$output .= '<div class="entry-thumbnail">';
				...
				$output .= '<div class="entry-summary-overlay">';
					$output .= '<div class="entry-featured-status">';
						$output .= vonzot_sample( get_the_content(), 33 );
