<?php
/**
 * Vonzot customizer colors preview functions
 *
 * @package WordPress
 * @subpackage Vonzot
 * @version 1.2.9
...
/**
 * Registers color schemes for Vonzot.
 *
 * Can be filtered with {@see 'vonzot_color_schemes'}.
 *
 ...
 */
function vonzot_get_color_schemes() {
	/**
	 * Filter the color schemes registered for use with Vonzot.
	 *
	 ...
	 * }
	 */
	return apply_filters( 'vonzot_color_schemes', array(

		'default' => array(
			'label'  => esc_html__( 'Default', 'vonzot' ),
			'colors' => array(
				'#ffffff', // body_bg
				'#ffffff', // page_bg
 ...
 */
function vonzot_get_color_scheme_css( $colors ) {
	...
	$border_color = vsprintf( 'rgba( %s, 0.08)', vonzot_hex_to_rgb( $strong_text_color ) );
	...
	***********************/
	...
	return apply_filters( 'vonzot_color_scheme_output', $output, $colors );
}
 ...
 * @return array $colors
 */
function vonzot_get_template_colors() {
 ...
 * Customizer preview.
 */
function vonzot_color_scheme_css_template() {

	$colors = vonzot_get_template_colors();
	?>
	<script type="text/html" id="tmpl-vonzot-color-scheme">
		<?php echo vonzot_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'vonzot_color_scheme_css_template' );