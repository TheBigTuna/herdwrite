 ...
 */
function vonzot_get_fonts_css( $fonts_args ) {
	...
	/* Body */
	$body_selectors = apply_filters( 'vonzot_body_font_family_selectors', vonzot_list_to_array( 'body, blockquote.wvc-testimonial-content, .tp-caption:not(h1):not(h2):not(h3):not(h4):not(h5)' ) );

	$body_selectors = vonzot_array_to_list( $body_selectors );
	...
	$menu_selectors = apply_filters( 'vonzot_menu_selectors', vonzot_list_to_array( '.nav-menu li, .cta-container' ) );

	$menu_selectors = vonzot_array_to_list( $menu_selectors );
	...
	/* Heading */
	$heading_family_selectors = apply_filters( 'vonzot_heading_family_selectors', vonzot_list_to_array( 'h1, h2, h3, h4, h5, h6, .post-title, .entry-title, h2.entry-title > .entry-link, h2.entry-title, .widget-title, .wvc-counter-text, .wvc-countdown-period, .event-date, .logo-text, .wvc-interactive-links, .wvc-interactive-overlays, .heading-font' ) );
	...
	$heading_family_selectors = vonzot_array_to_list( $heading_family_selectors );
	...
	return apply_filters( 'vonzot_fonts_css_output', $font_css, $fonts_args );
}
 ...
 * @return array $fonts
 */
function vonzot_get_template_fonts() {
 ...
 * Customizer preview.
 */
function vonzot_fonts_css_template() {

	$fonts = vonzot_get_template_fonts();
	?>
	<script type="text/html" id="tmpl-vonzot-fonts">
		<?php echo vonzot_get_fonts_css( $fonts ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'vonzot_fonts_css_template' );