<?php
/**
 * Vonzot Menu item custom fields example
 *
 ...
 * @since 0.1.0
 */
class Vonzot_Menu_Item_Custom_Fields {
	...
	public static function _save( $menu_id, $menu_item_db_id, $menu_item_args ) {
		...
		$meta_keys = apply_filters( 'vonzot_menu_meta_keys', array(
			'_menu-item-icon',
		...
		?>
			<p class="field-_mega-menu description description-wide">
				<label for="edit-_mega-menu-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php esc_html_e( 'Mega Menu (only available for first level items)', 'vonzot' ) ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-not-linked description description-wide">
				<label for="edit-_menu-item-not-linked-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php esc_html_e( 'Mega Menu 2nd level or dropdown item', 'vonzot' ) ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-hidden description description-wide">
				<label for="edit-_menu-item-hidden-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php esc_html_e( 'Hide item on mega menu (for mega menu 2nd level only)', 'vonzot' ) ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-button-style description description-wide">
				<label for="edit-_menu-item-button-style-<?php echo esc_attr( $item_id ) ?>">
					<select name="_menu-item-button-style[<?php echo esc_attr( $item_id ); ?>]">
						<option value=""><?php esc_html_e( 'None', 'vonzot' ); ?></option>
						<option value="primary" <?php selected( get_post_meta( $item_id, '_menu-item-button-style', true ), 'primary' ); ?>><?php esc_html_e( 'Primary', 'vonzot' ); ?></option>
						<option value="secondary" <?php selected( get_post_meta( $item_id, '_menu-item-button-style', true ), 'secondary' ); ?>><?php esc_html_e( 'Secondary', 'vonzot' ); ?></option>
					</select>
					<?php esc_html_e( 'Button Style (only available for first level items)', 'vonzot' ) ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-scroll description description-wide">
				<label for="edit-_menu-item-scroll-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php esc_html_e( 'Scroll to an anchor', 'vonzot' ) ?>
				</label>
			</p>
			<p class="field-_mega-menu-cols description description-wide">
					<label for="edit-_mega-menu-cols-<?php echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Mega Menu Columns', 'vonzot' ) ?></label>
						<br>
			...
			<p class="field-_mega-menu-tagline description description-wide">
				...
				<?php esc_html_e( 'Optional Mega Menu Tagline (only available for first level items)', 'vonzot' ) ?>
			</p>
			<?php if ( function_exists( 'wvc_get_fontawesome_icons' ) ) : ?>
				<?php
					$wvc_icons = wvc_get_fontawesome_icons();
				?>
				<p class="field-custom description description-wide vonzot-searchable-container">
					<label for="edit-_menu-item-icon-<?php echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Icon', 'vonzot' ) ?></label><br />
						<span><?php printf(
							'<select data-placeholder="%1$s" name="_menu-item-icon[%2$d]" class="vonzot-searchable edit-_menu-item-icon" id="edit-_menu-item-icon-%2$d">',
							esc_html__( 'None', 'vonzot' ),
							$item_id
						);
						echo '<option value="">' . esc_html__( 'None', 'vonzot' ) . '</option>';
				...
				<p class="field-_menu-item-icon-position description description-wide">
					<label for="edit-_menu-item-icon-position-<?php echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Icon position', 'vonzot' ) ?></label>
						<br>
						<select name="_menu-item-icon-position[<?php echo esc_attr( $item_id ); ?>]">
							<option value="before" <?php selected( get_post_meta( $item_id, '_menu-item-icon-position', true ), 'before' ); ?>><?php esc_html_e( 'before', 'vonzot' ); ?></option>
							<option value="after" <?php selected( get_post_meta( $item_id, '_menu-item-icon-position', true ), 'after' ); ?>><?php esc_html_e( 'after', 'vonzot' ); ?></option>
						</select>
				</p>
			<?php endif;

			echo apply_filters( 'vonzot_menu_meta_fields_markup', ob_get_clean(), $item_id );
	}
...
Vonzot_Menu_Item_Custom_Fields::init();
