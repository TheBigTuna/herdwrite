 ...
 */
function vonzot_set_default_mods() {
	...
	if ( is_file( get_parent_theme_file_path( 'config/' . vonzot_get_theme_slug() . '.dat' ) ) ) {
		$mod_filename = vonzot_get_theme_slug() . '.dat';
	}

	$file_content = vonzot_file_get_contents( 'config/' . $mod_filename );
	$theme_slug = ( is_child_theme() ) ? vonzot_get_theme_slug() . '_child' : vonzot_get_theme_slug();
	$theme_parent_slug = vonzot_get_theme_slug();
	...
	} elseif ( array() !== apply_filters( 'vonzot_default_mods_fallback_array', array() ) ) {
		
		/* Default mods in case the file import doesn't work */
		$mods = apply_filters( 'vonzot_default_mods_fallback_array', array() );
	}
	...
	$mods = apply_filters( 'vonzot_default_mods', $mods ); // filter default mods if needed

	foreach ( $mods as $key => $value ) {

		/* remove external URL to avoid hot linking */
		if ( vonzot_is_external_url( $value ) ) {
			$mods[ $key ] = '';
		}
...
add_action( 'after_setup_theme', 'vonzot_set_default_mods' );
 ...
 */
function vonzot_set_wvc_default_google_fonts( $settings ) {

	/* Get theme fonts */
	$theme_google_font_option = vonzot_get_option( 'fonts', 'google_fonts' );
...
add_filter( 'wvc_default_settings', 'vonzot_set_wvc_default_google_fonts' );
 ...
 * @return array $mods
 */
function vonzot_set_default_social_networks( $mods ) {
...
add_filter( 'vonzot_default_mods', 'vonzot_set_default_social_networks' );

/**
 * Define WooCommerce image sizes on theme activation
 *
 * Can be overwritten with the vonzot_woocommerce_thumbnail_sizes filter
 */
function vonzot_woocommerce_image_sizes() {
	...
	$woocommerce_thumbnails = apply_filters( 'vonzot_woocommerce_thumbnail_sizes', array(
			'catalog' => array(
				'width' 	=> '400',	// px
				'height'	=> '400',	// px
				'crop'	=> 1 		// true
...
add_action( 'after_switch_theme', 'vonzot_woocommerce_image_sizes', 1 );

/**
 * Set default WP options on theme activation
 */
function vonzot_default_wp_options_init() {
	if ( ! get_option( vonzot_get_theme_slug() . '_wp_options_init' ) ) {

		/**
		 * A custom hook to set default options on theme activation
		 */
		do_action( 'vonzot_wp_default_options_init' );

		/*
		 * Another custom hook to set default 3rd party plugin options on theme activation
		 */
		do_action( 'vonzot_plugins_default_options_init' );

		/* Default WP options */
		update_option( 'image_default_link_type', 'file' );

		/* Add option to flag that the default mods have been set */
		add_option( vonzot_get_theme_slug() . '_wp_options_init', true );

		update_option( 'wpb_js_gutenberg_disable', true );
	}
}
add_action( 'init', 'vonzot_default_wp_options_init' );
 ...
 * @return $mods
 */
function vonzot_set_default_customizer_images( $mods ) {

	$default_images = apply_filters( 'vonzot_default_image_mods', array( 'header', 'background' ) );

	foreach ( $default_images as $default_image ) {

		if ( vonzot_get_theme_uri( 'config/' . $default_image . '.jpg' ) ) {
			...
			$movefile = wp_upload_bits( $default_image . '.jpg', null, vonzot_file_get_contents( $filename ) );
			...
			if ( $movefile && isset( $movefile['url'] ) && empty( $movefile['error'] ) ) {
				...
				if ( $attach_id  ) {
					$mods[ $default_image . '_image'] = vonzot_get_theme_uri( 'config/' . $default_image . '.jpg' );
...
add_filter( 'vonzot_default_mods', 'vonzot_set_default_customizer_images' );