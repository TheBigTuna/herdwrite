 ...
 */
/* jshint -W062 */
/* global VonzotParams, WOW, AOS, WVC, Cookies, Event, objectFitImages, WVCBigText, WVCParams, VonzotYTVideoBg, Vimeo */

var VonzotUi = function( $ ) {
	...
	return {
		...
		init : function () {
			...
			this.mobileScreenBreakpoint = VonzotParams.mobileScreenBreakpoint;
			this.tabletScreenBreakpoint = VonzotParams.tabletScreenBreakpoint;
			this.notebookScreenBreakpoint = VonzotParams.notebookScreenBreakpoint;
			this.desktopScreenBreakpoint = VonzotParams.desktopScreenBreakpoint;
			this.desktopBigScreenBreakpoint = VonzotParams.desktopBigScreenBreakpoint;
		...
		setScreenClasses : function() {
			...
			var winWidth = $(window).width(),
				$body = $( 'body' ),
				breakpoints = [
					['desktop-screen desktop-big-screen', this.desktopBigScreenBreakpoint ],
				...
				allClasses = 'desktop-screen desktop-big-screen notebook-screen tablet-screen mobile-screen';
			...
			} else if ( winWidth > this.mobileScreenBreakpoint && winWidth <= this.tabletScreenBreakpoint) {
				$body.addClass( 'tablet-screen' );
			}

			else if ( winWidth > this.tabletScreenBreakpoint && winWidth <= this.notebookScreenBreakpoint) {
				$body.addClass( 'notebook-screen' );
			
			} else if ( winWidth > this.notebookScreenBreakpoint && winWidth <= this.desktopBigScreenBreakpoint ) {
		...
		setScreenClassesBak : function() {
			...
			if ( $windowWidth > this.desktopBigScreenBreakpoint ) {
				...
				$body.removeClass( 'notebook-screen tablet-screen mobile-screen' );
				$body.addClass( 'desktop-screen desktop-big-screen' );


			} else if ( $windowWidth > this.desktopScreenBreakpoint && $windowWidth < this.desktopBigScreenBreakpoint ) {
				
				$body.removeClass( 'desktop-big-screen desktop-screen notebook-screen tablet-screen mobile-screen' );
				$body.addClass( 'desktop-screen' );
			
			} else if ( $windowWidth > this.notebookScreenBreakpoint && $windowWidth < this.desktopScreenBreakpoint ) {
				
				$body.removeClass( 'desktop-big-screen desktop-screen notebook-screen tablet-screen mobile-screen' );
				$body.addClass( 'notebook-screen' );
			
			} else if ( $windowWidth > this.tabletScreenBreakpoint && $windowWidth < this.notebookScreenBreakpoint ) {
		...
		setClasses : function () {
			...
			if ( ( this.isMobile || 800 > $( window ).width() ) && ! VonzotParams.forceAnimationMobile ) {
		...
		loadingAnimation : function () {
			...
			if ( ! VonzotParams.defaultPageLoadingAnimation ) {
				return false;
			}
			...
			if ( $.isFunction( $.fn.stick_in_parent ) ) {
				if ( $( 'body' ).hasClass( 'single-work-layout-sidebar-left' ) || $( 'body' ).hasClass( 'single-work-layout-sidebar-right' ) ) {
					$( '.work-info-container' ).stick_in_parent( {
						offset_top : parseInt( VonzotParams.portfolioSidebarOffsetTop, 10 )
					} );
				}
			}
		},
		...
		mobileMenuBreakPoint : function () {
			...
			var $body = $( 'body' ),
				...
				breakpoint = VonzotParams.breakPoint;
			...
			if ( breakpoint > winWidth ) {
				...
				$( window ).trigger( 'vonzot_breakpoint' );
		...
		subMenuDirection : function() {
			...
			var $this,
				subMenuWidth = parseInt( VonzotParams.subMenuWidth, 10 ),
				bleed = 8;
		...
		centeredLogo : function () {
			...
			$( '<li class="logo-menu-item">' + VonzotParams.logoMarkup + '</li>' ).insertAfter( '#site-navigation-primary-desktop > li:nth-child(' + middleItemCount + ')' );
		},
		...
		stickyMenu : function ( scrollTop ) {
			...
			var scrollPoint,
				menuOffset = parseInt( VonzotParams.menuOffset, 10 );

			scrollTop = scrollTop || 0;

			scrollPoint = parseInt( VonzotParams.stickyMenuScrollPoint, 10 ) || $( 'body' ).offset().top;
			...
			if ( menuOffset ) {
				...
				if ( '100%' === VonzotParams.menuOffset ) {

					scrollPoint = $( window ).height() - parseInt( VonzotParams.desktopMenuHeight, 10 );

				} else {
					scrollPoint = menuOffset - parseInt( VonzotParams.desktopMenuHeight, 10 );
				}
			}

			if ( $( '#top-bar-block' ).length  ) {
				scrollPoint = $( '#top-bar-block' ).offset().top + $( '#top-bar-block' ).outerHeight();
			}

			if ( 'soft' === VonzotParams.stickyMenuType ) {
			...
			} else if ( 'hard' === VonzotParams.stickyMenuType ) {
		...
		parallax : function () {
			...
			var smallScreen = ( ( 800 > $( window ).width() || this.isMobile ) && VonzotParams.parallaxNoSmallScreen );

			if ( ! smallScreen ) {
				$( '.parallax' ).jarallax( {
					noAndroid : VonzotParams.parallaxNoAndroid,
					noIos : VonzotParams.parallaxNoIos
				} );
			}
		},
		...
		toggleMenu : function () {
			$( document ).on( 'click', '.toggle-mobile-menu', function( event ) {
				...
				$( window ).trigger( 'vonzot_mobile_menu_toggle_button_click', [ $( this ) ] );
				$( 'body' ).toggleClass( 'mobile-menu-toggle' );
			} );

			$( document ).on( 'click', '.toggle-side-panel', function( event ) {
				event.preventDefault();
				$( window ).trigger( 'vonzot_side_panel_toggle_button_click', [ $( this ) ] );
				$( 'body' ).toggleClass( 'side-panel-toggle' );
			} );

			$( document ).on( 'click', '.toggle-offcanvas-menu', function( event ) {
				event.preventDefault();
				$( window ).trigger( 'vonzot_offcanvas_menu_toggle_button_click', [ $( this ) ] );
			...
			$( document ).on( 'click', '.toggle-overlay-menu', function( event ) {
				...
				$( window ).trigger( 'vonzot_overlay_menu_toggle_button_click', [ $( this ) ] );
			} );

			$( document ).on( 'click', '.toggle-lateral-menu', function( event ) {
				event.preventDefault();
				$( window ).trigger( 'vonzot_lateral_menu_toggle_button_click', [ $( this ) ] );
		...
		toggleSearchForm : function () {
			$( document ).on( 'click', '.toggle-search', function() {
				$( window ).trigger( 'vonzot_searchform_toggle' );
		...
		flexSlider : function () {
			...
			if ( $.isFunction( $.flexslider ) ) {
				...
				$( '.entry-slider' ).flexslider( {
					animation: VonzotParams.entrySliderAnimation,
					slideshow : true,
					slideshowSpeed : 4000
				} );
			}
		...
		lightbox : function () {
			...
			if ( 'fancybox' === VonzotParams.lightbox ) {

				$( selectors ).fancybox( VonzotParams.fancyboxSettings );
				...
				$( '.gallery-quickview, .wvc-gallery-quickview' ).unbind().on( 'click', function() {
					...
					params = VonzotParams.fancyboxSettings;
				...
				$( '.woocommerce-product-gallery__trigger' ).unbind().on( 'click', function() {
					...
					$.fancybox.open( quickviewData, VonzotParams.fancyboxSettings );
					return false;
				} );
				...
				$( '.lightbox-video' ).fancybox( {
					...
					beforeLoad : function() {
						...
						VonzotUi.pausePlayers();
					},

					afterLoad : function() {

						VonzotUi.lightboxVideoAfterLoad();
					},

					afterClose : function() {

						VonzotUi.restartVideoBackgrounds();
					}
				} );
			}
		},
		...
		lightboxVideoAfterLoad : function () {
			...
			var $iframe = $( '.fancybox-iframe' ).contents(),
				...
				accentColor = VonzotParams.accentColor;
			...
			if ( $video.length ) {
				...
				$head.append( $( '<link/>',
					{
						rel: 'stylesheet',
						href: VonzotParams.mediaelementCssUri,
						type: 'text/css'
					} )
				);
				...
				$head.append( $( '<link/>',
					{
						rel: 'stylesheet',
						href: VonzotParams.fancyboxMediaelementCssUri,
						type: 'text/css'
					} )
				);
		...
		scrollToMainContent : function () {
			...
			}, parseInt( VonzotParams.smoothScrollSpeed, 10 ), VonzotParams.smoothScrollEase, function() {
		...
		smoothScroll : function( href ) {
			...
			if ( -1 !== href.indexOf( '#' ) ) {
				...
				if ( $( '#' + hash ).length ) {
					...
					if ( 'hard' === VonzotParams.stickyMenuType && ! $( 'body' ).hasClass( 'sticky-menu-transparent' ) && ! $( '#' + hash ).hasClass( 'wvc-row-full-height' ) ) {
						scrollOffset += parseFloat( VonzotParams.stickyMenuHeight, 10 );
					}
		...
		wowAnimate : function () {
			...
			var wowAnimate,
				doWow = ( VonzotParams.forceAnimationMobile || ( ! this.isMobile && 800 < $( window ).width() ) );

			if ( doWow && 'undefined' !==  typeof WOW ) {
				wowAnimate = new WOW( { offset : VonzotParams.WOWAnimationOffset } ); // init wow for CSS animation
				wowAnimate.init();
			}
		},
		...
		liveSearch : function () {
			...
			$( '.live-search-form' ).each( function() {
				var $formContainer = $( this ),
					...
					timer = null,
					$resultContainer,
					action = 'vonzot_ajax_live_search',
					result;

				if ( $form.hasClass( 'woocommerce-product-search' ) ) {
					action = 'vonzot_ajax_woocommerce_live_search';
				}
			...
			if ( ! this.isMobile ) {
				...
				$( document ).on( 'added_to_cart', function( event, fragments, cart_hash, $button ) {
					...
					if ( $button.hasClass( 'wvc-ati-add-to-cart-button' ) || $button.hasClass( 'wpm-add-to-cart-button' ) || $button.hasClass( 'wolf-release-add-to-cart' ) || $button.hasClass( 'product-add-to-cart' ) ) {
						...
						$tipspan.attr( 'title', VonzotParams.l10n.addedToCart );
						...
						setTimeout( function() {
							$tipspan.data( 'tooltipsy' ).hide();
							$tipspan.data( 'tooltipsy' ).destroy();
							$tipspan.attr( 'title', VonzotParams.l10n.addToCart );
							$tipspan.tooltipsy();
		...
		setInternalLinkClass : function () {
			...
			var siteURL = VonzotParams.siteUrl,
				$internalLinks,
				regEx = '';

			$.each( VonzotParams.allowedMimeTypes, function( index, value ) {
				regEx += '|' + value;
			} );

			regEx = $.trim( regEx ).substring(1);

			siteURL = VonzotParams.siteUrl;
			...
			if ( VonzotParams.isWooCommerce ) {

				/*
				When WC pages aren't set the WC pages variables will return the siteURL
				Be sure it is not the same !!
				 */
				if ( this.untrailingSlashit( siteURL ) !== this.untrailingSlashit( VonzotParams.WooCommerceCartUrl ) ) {
					$( 'a[href^="' + VonzotParams.WooCommerceCartUrl + '"]' ).removeClass( 'internal-link' );
				}

				if ( this.untrailingSlashit( siteURL ) !== this.untrailingSlashit( VonzotParams.WooCommerceAccountUrl ) ) {
					$( 'a[href^="' + VonzotParams.WooCommerceAccountUrl + '"]' ).removeClass( 'internal-link' );
				}

				if ( this.untrailingSlashit( siteURL ) !== this.untrailingSlashit( VonzotParams.WooCommerceCheckoutUrl ) ) {
					$( 'a[href^="' + VonzotParams.WooCommerceCheckoutUrl + '"]' ).removeClass( 'internal-link' );
				}
		...
		transitionCosmetic : function() {
			...
			if ( ! VonzotParams.defaultPageTransitionAnimation ) {
				return false;
			}

			var _this = this;

			if ( VonzotParams.isAjaxNav ) {
				return;
			}
			...
			$( document ).on( 'click', '.internal-link:not(.disabled)', function( event ) {
				...
				if ( ! event.ctrlKey ) {
					...
					if ( VonzotParams.hasLoadingOverlay ) {
		...
		videoThumbnailPlayOnHover : function() {
			...
			$( 'iframe.youtube-bg', itemsContainer ).each( function() {
				if ( 'undefined' !== typeof VonzotYTVideoBg && VonzotYTVideoBg.players[ $( this ).data( 'youtube-video-id' ) ] ) {
					VonzotYTVideoBg.players[$( this ).data( 'youtube-video-id' )].pauseVideo();
				}
			} );
				...
				if ( $iframe.length ) {
					...
					if ( $iframe.hasClass( 'youtube-bg' ) ) {
						...
						$( this ).on( 'mouseenter', function() {
							...
							if ( 'undefined' !== typeof VonzotYTVideoBg && VonzotYTVideoBg.players[ YTPlayerId ] ) {
								VonzotYTVideoBg.players[YTPlayerId].playVideo();
							}
						} );
						...
						$( this ).on( 'mouseleave', function() {
							...
							setTimeout( function() {
								if ( 'undefined' !== typeof VonzotYTVideoBg && VonzotYTVideoBg.players[ YTPlayerId ] ) {
									VonzotYTVideoBg.players[YTPlayerId].pauseVideo();
								}
							}, 500 );
						} );
		...
		navPlayer : function () {
			$( document ).on( 'click', '.nav-play-button', function() {
				...
				var $btn = $( this ),
					...
					audioId = $audio.attr( 'id' ),
					audio = document.getElementById( audioId ),
					playText = VonzotParams.l10n.playText,
					pauseText = VonzotParams.l10n.pauseText;
		...
		topbarClose : function() {
			if ( $( '#top-bar-block' ).length ) {
				...
				$container.addClass( 'wvc-font-' + $container.find( '.wvc-row' ).first().data( 'font-color' ) );
		...
		pageLoad : function () {
			...
			delay = VonzotParams.pageTransitionDelayAfter;
			...
			}, parseInt( VonzotParams.pageLoadedDelay, 10 ) );
		}
	};
...
( function( $ ) {
	...
	if ( 'undefined' !== typeof WVC ) {
		 ...
		 */
		WVC.lightbox = VonzotUi.lightbox;

		/**
		 * Overwrite WVC video opener
		 */
		WVC.videoOpener = VonzotUi.videoOpener;

		/**
		 * Overwrite toolbar offset calculation
		 */
		WVC.getToolBarOffset = VonzotUi.getToolBarOffset;
	}
