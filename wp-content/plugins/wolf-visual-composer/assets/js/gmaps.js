...
var WVCGmaps = function( $ ) {
	...
	return {
		 ...
		 */
		scrollFix : function () {
			$( '.wvc-gmaps-container' ).click( function () {
				$( this ).find( '.wvc-gmaps' ).css( 'pointer-events', 'auto' );
			} );

			$( '.wvc-gmaps-container' ).mouseleave( function() {
				$( this ).find( '.wvc-gmaps' ).css( 'pointer-events', 'none' );
			} );
		}
	};
...
( function( $ ) {
	...
	if ( 'undefined' !== google && WVCParams.googleMapApiKey ) {
		google.maps.event.addDomListener( window, 'load', WVCGmaps.init() );
	}
